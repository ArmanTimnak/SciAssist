...
def gcd_calc(numbers):
    gcd_result = numbers[0]
    for i in range(1, len(numbers)):
        gcd_result = math.gcd(gcd_result, numbers[i])
    return gcd_result

async def gcd(update: Update, context: CallbackContext) -> None:
    ...
    await update.message.reply_text(f"The gcd of the numbers provided is: {gcd_calc(input_args)}")
